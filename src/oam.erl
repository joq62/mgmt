...
preload_dbase()-> 
    gen_server:call(?MODULE, {preload_dbase},infinity).
...
init([]) ->
    % check if local dbase is started Assum that dbase running
    {pong,_,_}=dbase:ping(),
...
handle_call({preload_dbase},_From,State) ->
    Reply=rpc:call(node(),oam_lib,preload_dbase,[],20*1000),
    {reply, Reply, State};

handle_call({init_dbases},_From,State) ->
    Reply=rpc:call(node(),oam_lib_dbase,init,[],20*1000),
    {reply, Reply, State};
